...
export default {
  ...
  methods: {
     ...
     * @param {*} isEl 是否为元素
     */
    getBoundingClientRect(target, isEl=true) {
      const obj = isEl ? target.getBoundingClientRect() : target;
      const rect = {};
      for (const key in obj) {
        const item = obj[key];
        if (!isNaN(obj[key])) {
          rect[key] = obj[key] / this.scaleRate
        }
      }
      return rect;
    },
    handleDrag(event) {
      ...
      switch (event.type) {
        case "dragstart": {
          ...
          let clientRect = this.getBoundingClientRect(event.target);
          if (this.renderRtl) {
        ...
        case "dragmove": {
          ...
          //                        Add rtl support
          // console.log('dragmove',this.renderRtl,position,coreEvent.deltaX, this.calcXY(dragging.top, dragging.left));
          const {scaleRate} = this
          // todoc 换算鼠标移动距离
          if (this.renderRtl) {
            newPosition.left = this.dragging.left/1 - coreEvent.deltaX/scaleRate;
          } else {
            newPosition.left = this.dragging.left/1 + coreEvent.deltaX/scaleRate;
          }
          newPosition.top = this.dragging.top/1 + coreEvent.deltaY/scaleRate;
    ...
    tryMakeDraggable: function () {
      ...
      if (this.draggable && !this.static) {
        ...
        if (!this.dragEventSet) {
          ...
          // todoc 定位元素设置缩放比例后那些内容有变化
          // this.interactObj.on("mouseenter", function (event) {
          //   this.flagObj =
          //     /* this.getBoundingClientRect(event.target);  */ event.target.getBoundingClientRect();
          //   const current = event.target.getBoundingClientRect();
          //   const parent = event.target.offsetParent.getBoundingClientRect();
          //   const current2 = this.getBoundingClientRect(event.target);
          //   const parent2 = this.offsetParent.getBoundingClientRect(event.target);
          //   // console.table(current);
          //   console.table({
          //     top: current.top - parent.top,
          //     left: current.left - parent.left,
          //   });
          //   // console.table(current2);
          //   // console.table({
          //   //   top: current2.top - parent2.top,
          //   //   left: current2.left - parent2.left,
          //   // });
          // });
        }
      } else {
    ...
    autoSize: function () {
      ...
      let newSize = this.getBoundingClientRect(this.$slots.default[0].elm);
